...
reset_param(const MethylList& met, const GlobalStatistics& gstat)
{
  ...
  for (uint i=0; i!=met.pos_size(); ++i) {
    ...
    EmitProb[CPG_UP2][ii] = pup;
    EmitProb[CPG_DOWN2][ii] = pdown;
    EmitProb[CPG_NOCHANGE2][ii] = pnochange;
    EmitProb[GAP_UP][ii] = NEG_INF;
    EmitProb[GAP_DOWN][ii] = NEG_INF;
    EmitProb[GAP_NOCHANGE][ii] = NEG_INF;
    EmitProb[GAP_UP2][ii] = NEG_INF;
    EmitProb[GAP_DOWN2][ii] = NEG_INF;
    EmitProb[GAP_NOCHANGE2][ii] = NEG_INF;
  }
...
dbase(std::ofstream& ofs, const MethylList& met) 
{
  ...
  for (uint i=0; i!=met.pos_size(); ++i) {
    ...
    for (uint j=0; j!=NCPGState/2; ++j) 
      ofs << "\t" << formatval("%.8e", ppr_[j][ii] + ppr_[j+NCPGState/2][ii]);
    ofs << endl; 
  }
}
...
dregion_ratio(std::ofstream& ofs, const MethylList& met, ValueType thsh) 
{
  ...
  vector<ValueType> tbl_up_cgi(met.pos_size()); 
  vector<ValueType> tbl_down_cgi(met.pos_size()); 
  vector<ValueType> tbl_nc_cgi(met.pos_size()); 
  ...
  while (true) { // each time find the best path  
    ...
    tbl_up_cgi.assign(met.pos_size(), NEG_INF);
    tbl_down_cgi.assign(met.pos_size(), NEG_INF);
    tbl_nc_cgi.assign(met.pos_size(), NEG_INF);
	...
	val_up_cgi = TransProb[CPG_UP2][CPG_UP2] + EmitProb[CPG_UP2][ii] ;
	val_down_cgi = TransProb[CPG_DOWN2][CPG_DOWN2] + EmitProb[CPG_DOWN2][ii];
	val_nc_cgi = TransProb[CPG_NOCHANGE2][CPG_NOCHANGE2] + EmitProb[CPG_NOCHANGE2][ii];
	...
	v_up_cgi = TransProb[GAP_NOCHANGE][CPG_UP2] + EmitProb[CPG_UP2][ii];
	v_down_cgi = TransProb[GAP_NOCHANGE][CPG_DOWN2] + EmitProb[CPG_DOWN2][ii];
